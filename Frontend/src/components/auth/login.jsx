  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-2 sm:px-4" style={{ backgroundImage: "url('https://res.cloudinary.com/dgsmgz8zl/image/upload/v1759471113/Gemini_Generated_Image_mylwunmylwunmylw_zac7dw.png')", backgroundSize: "cover", backgroundPosition: "center" }}>
      <div className="w-full max-w-md bg-white shadow-xl rounded-lg sm:rounded-xl p-4 sm:p-6 lg:p-8">
        ...
        <h1 className="text-center text-2xl sm:text-3xl font-bold text-indigo-600 mb-3 sm:mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
          ...
          <button type="submit" className="w-full bg-brand hover:bg-brand-dark text-white rounded-lg py-2 sm:py-2.5 transition text-sm sm:text-base font-medium disabled:opacity-50 disabled:cursor-not-allowed" disabled={isSubmitting}>
        ...
        <div className="mt-4 sm:mt-6 flex flex-col items-center space-y-2 text-xs sm:text-sm text-gray-600">
          <div className="flex gap-1">
            New user?
            <Link to="/register" onClick={() => window.location.href = "/register"} className="text-brand hover:text-brand-dark font-medium">Create account</Link>
          </div>
          <Link to="/forgot-password" onClick={() => window.location.href = "/forgot-password"} className="text-gray-500 hover:text-gray-700">Forgot password?</Link>
        </div>
      </div>
    </div>
  );
